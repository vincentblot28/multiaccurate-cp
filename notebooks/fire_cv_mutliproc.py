...
# %%
BASE_DIR = "/mnt/ssd/multiaccurate/data/fire"
...
# %%
random_idx = np.random.randint(0, len(load_cal_pred_probas), 1000)
all_pred_probas = np.concatenate([load_cal_pred_probas, load_test_pred_probas])[random_idx]
all_res_emb = np.concatenate([load_cal_res_emb, load_test_res_emb])[random_idx]
all_labels = np.concatenate([load_cal_labels, load_test_labels])[random_idx]
...
# %%
with open("fire_thresholds_aacrc_multiproc.pkl", "wb") as f:
        pickle.dump(all_ths_res, f)
with open("fire_thresholds_crc_multiproc.pkl", "wb") as f:
    pickle.dump(all_ths_crc, f)
with open("polyp_recalls_precisions_multiproc.pkl", "wb") as f:
    pickle.dump(
        {
